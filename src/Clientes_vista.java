 ...
 * @author Bernardo SJ
 */
public class Clientes_vista extends JFrame implements ActionListener {
    ...
    JCheckBox muñequerasnike = new JCheckBox("Balon Pitch Team\n$" + usuarios.registro.productos.get(3).getPrecio(), false);
    ...
    JCheckBox oferta1 = new JCheckBox("Tenis Air Max y Balon King force  $200");
    JCheckBox oferta2 = new JCheckBox("Muñequeras Nike Swoosh  y Cantimplora negro/blanco $200");
    JLabel apartado = new JLabel("Apartado de productos");
    JButton btnApartar = new JButton("Apartar productos");
    JLabel cantidado1 = new JLabel("Cantidad:");
    JLabel cantidado2 = new JLabel("Cantidad:");
    ...
    void cargarCantidades() {
        ...
        cantidado1.setText("Catidad: " + usuarios.registro.productos.get(12).getCantidad());
        cantidado2.setText("Cantidad: " + usuarios.registro.productos.get(13).getCantidad());
    }

    public Clientes_vista() {
        super("Vista Clientes");
        setSize(1350, 600);
        ...
        btnApartar.addActionListener(this);
        if (usuarios.registro.clientes.get(Inicio.pos).getDineroApartadoUltimaCompra() > 0) {
            btnApartar.setText("Pagar apartado");
        } else {
            btnApartar.setText("Apartar productos");
        }
        ...
        getContentPane().add(ofertas);
        ofertas.setBounds(50, 430, 50, 14);
        getContentPane().add(oferta1);
        oferta1.setBounds(30, 470, 260, 23);
        oferta1.setOpaque(false);
        getContentPane().add(oferta2);
        oferta2.setOpaque(false);
        oferta2.setBounds(30, 500, 380, 23);
        getContentPane().add(apartado);
        apartado.setBounds(520, 420, 130, 14);
        getContentPane().add(btnApartar);
        btnApartar.setBounds(520, 450, 140, 23);
        getContentPane().add(cantidado1);
        cantidado1.setBounds(305, 470, 90, 14);
        getContentPane().add(cantidado2);
        cantidado2.setBounds(415, 500, 70, 14);
        cargarCantidades();
        show();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (ev == btnRegistrar) {
            if (area.getText().equals("")) {
                ...
                if (oferta1.isSelected()) {
                    int unidadesac = usuarios.registro.productos.get(12).getCantidad();
                    usuarios.registro.productos.get(12).setCantidad(unidadesac - 1);
                }
                if (oferta2.isSelected()) {
                    int unidadesac = usuarios.registro.productos.get(13).getCantidad();
                    usuarios.registro.productos.get(13).setCantidad(unidadesac - 1);
                }
        ...
        if (ev == btnAñadir) {
            ...
            if (oferta1.isSelected()) {
                if (usuarios.registro.productos.get(12).getCantidad() == 0) {
                    JOptionPane.showMessageDialog(null, "Lo siento no quedan unidades de la oferta de " + usuarios.registro.productos.get(12).getNombre());
                    sePuede = false;
                } else {
                    unidadesT++;
                    total += 200;
                    res += "Producto " + usuarios.registro.productos.get(12).getNombre() + " total $200\n";
                }
            }
            if (oferta2.isSelected()) {
                if (usuarios.registro.productos.get(13).getCantidad() == 0) {
                    JOptionPane.showMessageDialog(null, "Lo siento no quedan unidades de la oferta de " + usuarios.registro.productos.get(13).getNombre());
                    sePuede = false;
                } else {
                    unidadesT++;
                    total += 200;
                    res += "Producto " + usuarios.registro.productos.get(13).getNombre() + " total $200\n";
                }
            }
            if (sePuede) {
                area.setText(res);
        ...
        if (ev == btnApartar) {
            if (btnApartar.getText().equals("Apartar productos")) {
                if (area.getText().equals("")) {
                    JOptionPane.showMessageDialog(null, "Para realizar un apartado debes presionar el boton Añadir compra");
                } else {
                    if (tenisnike.isSelected()) {
                        int unidades = Integer.parseInt(ctenisnike.getSelectedItem().toString());
                        int unidadesac = usuarios.registro.productos.get(0).getCantidad();
                    ...
                    usuarios.registro.ganancias += (unidadesT * 50);
                    cargarCantidades();
                    JOptionPane.showMessageDialog(null, "Tu total a pagar es  " + total);
                    btnApartar.setText("Pagar apartado");
                    usuarios.registro.clientes.get(Inicio.pos).setUltimaApartadoCompra(res);
                    usuarios.registro.clientes.get(Inicio.pos).setDineroApartadoUltimaCompra(total);
                    area.setText("");
                    usuarios.registro.guardar();
                }
            } else {
                int op = JOptionPane.showConfirmDialog(null, "¿Quieres pagar tu adeudo?", "ADVERTENCIA", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                if (op == 0) {
                    JOptionPane.showMessageDialog(null, "Tu total es " + usuarios.registro.clientes.get(Inicio.pos).getDineroApartadoUltimaCompra());
                    usuarios.registro.clientes.get(Inicio.pos).setDineroApartadoUltimaCompra(0f);
                    usuarios.registro.clientes.get(Inicio.pos).setUltimaApartadoCompra("");
                    btnApartar.setText("Apartar productos");
                    usuarios.registro.guardar();
                }
            }
